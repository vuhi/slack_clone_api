...
from api_core.apps.type import JWTConfig, IToken
...
class JWTTokenAuthentication(authentication.BaseAuthentication):
    ...
    def __init__(
        self,
        jwt_conf: JWTConfig = Provide['config.JWT_TOKEN'],
        token_service: IToken = Provide['token_service'],
        *args,
        **kwargs
    ):
    ...
    def authenticate(self, request: Request):
        ...
        try:
            claims = self.token_service.decode(raw_token)
            # TODO: change it to user_service
            user = self.user_model.service.get_active_user(id=claims[self.token_service.CLAIM.ID])  # type: ignore
            return user, None
